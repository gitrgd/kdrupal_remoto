...
class SliderController extends ControllerBase {
  ...
  public function gavias_sl_sliders_list($gid){
    ...
    foreach ($results as $row) {
      ...
      $tmp[] = t('<a href="@link_1">Edit Silder</a> | <a href="@link_2">Duplicate</a> | <a href="@link_3">Delete</a>', array(
          '@link_1' => Url::fromRoute('gavias_sl_sliders.admin.form', array('sid' => $row->id, 'gid' => $gid)),
          '@link_2' => Url::fromRoute('gavias_sl_sliders.admin.duplicate', array('id' => $row->id)),
          '@link_3' => Url::fromRoute('gavias_sl_group.admin.delete', array('sid' => $row->id, 'gid' => $gid, 'action' => 'slider'))
      ));
      $rows[] = $tmp;
    }
    return array(
      ...
      '#header' => $header,
      '#rows' => $rows,
      '#empty' => t('No Slider available. <a href="@link">Add Slider</a>.', array('@link' => Url::fromRoute('gavias_sl_sliders.admin.form', array('sid'=>0, 'gid'=>$gid)))),
    );
  }
  ...
  public function gavias_sl_sliders_edit($gid=0, $sid=0) {
    ...
    $abs_url_save = Url::fromRoute('gavias_sl_sliders.admin.save', array(), array('absolute' => FALSE));
    $abs_url_edit = Url::fromRoute('gavias_sl_sliders.admin.form', array('gid'=>$gid, 'sid'=>$sid), array('absolute' => FALSE));

    $abs_url_config = Url::fromRoute('gavias_sliderlayer.admin.get_images_upload', array(), array('absolute' => FALSE));
  ...
  public function gavias_sliderlayer_save(){
    ...
    if($sid > 0){
      ...
      $abs_url_edit = Url::fromRoute('gavias_sl_sliders.admin.form', array('gid'=>$gid, 'sid'=>$sid), array('absolute' => TRUE));
      $result = array(
        'data' => 'insert saved',
        'action' => 'edit',
